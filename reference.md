...
**contactUs:** `Optional<ContactElement>` — Contact us section of payment link page. If omitted, this block is enabled at display order 11.
...
**logo:** `Optional<Element>` — Logo section of payment link page. If omitted, this block is enabled at display order 1, and the logo image is resolved from the paypoint's entry logo.
...
**messageBeforePaying:** `Optional<LabelElement>` — Message section of payment link page. If omitted, this block is enabled at display order 5.
...
**notes:** `Optional<NoteElement>` — Notes section of payment link page. If omitted, this block is enabled at display order 10.
...
**page:** `Optional<PageElement>` — Page header section of payment link page. If omitted, this block is enabled at display order 2.
...
**paymentButton:** `Optional<LabelElement>` — Payment button section of payment link page. If omitted, this block is enabled at display order 6, with the label "Pay Now".
...
**paymentMethods:** `Optional<MethodElement>` — Payment methods section of payment link page. If omitted, this block is enabled at display order 3, with all payment methods enabled except RDC.
...
**review:** `Optional<HeaderElement>` — Review section of payment link page. If omitted, this block is enabled at display order 4.
...
**settings:** `Optional<PagelinkSetting>` — Settings section of payment link page. If omitted, defaults are applied, including page color `#10a0e3` and language `en`.
...
**billId:** `Integer` — The Payabli ID for the bill.
...
**amountFixed:** `Optional<String>` — Indicates whether customer can modify the payment amount. A value of `true` means the amount isn't modifiable, a value `false` means the payor can modify the amount to pay.
...
<details><summary><code>client.query.listDevices(entry) -> QueryDeviceResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.query.listDevicesOrg(orgId) -> QueryDeviceResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.query.listTransfersOutOrg(orgId) -> TransferOutQueryResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.query.listTransfersOutPaypoint(entry) -> TransferOutQueryResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.query.listTransferDetailsOut(entry, transferId) -> TransferOutDetailQueryResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.organization.editOrganization(orgIdPathParam, request) -> EditOrganizationResponse</code></summary>
<dl>
<dd>
...
**orgIdPathParam:** `Integer` — The numeric identifier for organization, assigned by Payabli.
...
**vendorId:** `Long` — ID of the vendor to call. Must be active and belong to the entrypoint in the path.
...
**phone:** `Optional<String>` — Vendor phone number to call, digits only. Optional. When omitted, Payabli uses the phone number on the vendor's record. If the vendor has no phone on record, the request returns an error.
...
**enrichmentId:** `Optional<String>` — ID of the originating enrichment run to associate with this call. Optional. When omitted, Payabli generates a standalone call schedule and skips the enrichment lookup. The bill due-date check only runs when both `enrichmentId` and `billId` are supplied.
...
**billId:** `Optional<Long>` — Bill ID used for the due-date check. When the bill is due in fewer than three days, the call is skipped and the fallback method is applied. Only evaluated when `enrichmentId` is also supplied.
...
**fallbackMethod:** `Optional<String>` — Payment method to apply to the vendor record if the call can't determine a preference or all retries are exhausted. Values are `check` (the default) or `managed`.
...
**maxRetries:** `Optional<Integer>` — Number of times to retry the call if the vendor doesn't answer. Defaults to 3. Maximum is 5. The get outreach call status response reports this value as `maxAttempts`.
...
**timezone:** `Optional<String>` — IANA timezone identifier used to schedule the call in the vendor's local time. Defaults to `America/New_York`.
...
**sendNow:** `Optional<Boolean>` — When `true`, dispatches the call immediately and bypasses the business-hours window and the bill due-date check. Defaults to `false`.
...
**amount:** `Double` — The amount to deposit, in dollars. Must be greater than zero.
...
**paypointId:** `Optional<Long>` — The paypoint ID. Optional if the entry point uniquely identifies the paypoint.
...
**sameDayAch:** `Optional<Boolean>` — When `true` and the request is submitted before 2 PM ET, the deposit processes as same-day ACH. If the request is submitted after 2 PM ET, it processes as standard ACH regardless of this flag.
...
## Billing
<details><summary><code>client.billing.listProfiles(orgId) -> BillingProfileQueryResponse</code></summary>
<dl>
<dd>
...
**orgId:** `Long` — The organization's numeric identifier.
...
**profileName:** `Optional<String>` — Filter to profiles whose name contains this string.
...
**serviceVertical:** `Optional<Integer>` 
...
**profileId:** `Optional<Long>` — Filter to a single profile by its identifier.
...
**fromRecord:** `Optional<Long>` — Zero-based offset into the result set. Defaults to `0`.
...
<details><summary><code>client.billing.getProfile(serviceGroup, entityType, entityId) -> BillingProfileResponse</code></summary>
<dl>
<dd>
...
**serviceGroup:** `GetProfileBillingRequestServiceGroup` 
...
**entityType:** `GetProfileBillingRequestEntityType` 
...
**entityId:** `Long` — The numeric identifier of the entity.
